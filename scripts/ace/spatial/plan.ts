...
manifest.scenes.forEach((scene, i) => {
  ...
  console.log(`  ${String(i + 1)}. ${scene.id}  [${scene.start.toFixed(2)} → ${scene.end.toFixed(2)}]`);
  console.log(`     mouvement   : ${scene.camera.move ?? "sur mesure"} — ${travel.toFixed(2)} unité(s)`);
  console.log(`     focale      : ${String(scene.camera.fovFrom)}° → ${String(scene.camera.fovTo)}°`);
  console.log(`     profondeur  : relief ${String(scene.depth.strength)}${scene.depthMap ? "" : "  ⚠ AUCUNE CARTE"}`);
  console.log(`     implantation: z = ${(origins[i] ?? 0).toFixed(2)} · cadrage le plus large à ${framing.distance.toFixed(2)}`);
  if (scene.transitionOut) {
    console.log(`     raccord     : ${scene.transitionOut.type} (${String(scene.transitionOut.duration)})`);
