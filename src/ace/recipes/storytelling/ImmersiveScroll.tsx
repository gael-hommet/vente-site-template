        ...
        >
          {c.media ? (
            ...
            <div
              aria-hidden
              className="absolute inset-0 -z-10"
              style={{ background: i % 2 ? "var(--surface-2)" : "var(--background)" }}
            />
          )}
          <div className={cn("mx-auto w-full max-w-5xl px-6", c.media && "text-white")}>
            {c.eyebrow ? (
              <p
                className={cn(
                  "mb-4 font-mono text-[0.8125rem] tracking-[0.12em] uppercase",
                  c.media ? "text-white/75" : "text-muted",
                )}
              >
                {c.eyebrow}
              </p>
            ) : null}
            <h2 className="font-display max-w-4xl text-[clamp(2.25rem,6vw,5rem)] leading-[1.05] font-light text-balance">
              {c.title}
            </h2>
            <p
              className={cn(
                "mt-6 max-w-2xl text-lg leading-relaxed",
                c.media ? "text-white/85" : "text-muted",
              )}
            >
              {c.body}
            </p>
