            ...
            >
              {item.media ? (
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(150deg, color-mix(in oklch, var(--brand) 24%, var(--surface-2)), var(--surface-3))",
                  }}
                />
              )}
            </div>
            <div className={cn(reversed && "md:order-1")}>
              <span className="text-muted font-mono text-[0.75rem] tracking-[0.12em] tabular-nums">
                {String(i + 1).padStart(2, "0")} / {String(items.length).padStart(2, "0")}
              </span>
              <h3 className="font-display group-hover:text-brand mt-3 text-[clamp(1.75rem,3.5vw,3rem)] leading-tight font-light transition-colors">
                {item.title}
              </h3>
              {item.meta?.length ? (
                <p className="text-muted mt-3 font-mono text-[0.75rem] tracking-[0.06em] uppercase tabular-nums">
                  {item.meta.join(" · ")}
                </p>
              ) : null}
            </div>
