...
const meta = (id: string, family: RecipeMeta["family"], title: string, description: string, tags: readonly string[]) =>
  ({ id, family, title, description, tags }) as const;

const heroes = createRegistry<Recipe<HeroProps>>("recipes:hero", [
  { ...meta("typographic", "hero", "Typographique", "Le titre est l'image, sans média.", ["typography-first", "no-media"]), Component: TypographicHero },
  { ...meta("media-first", "hero", "Média plein cadre", "Média plein écran + voile, immersif.", ["media-first", "immersive"]), Component: MediaFirstHero },
  { ...meta("split-narrative", "hero", "Récit scindé", "Récit à gauche, panneau média à droite.", ["split", "media"]), Component: SplitNarrativeHero },
]);

const navigation = createRegistry<Recipe<NavProps>>("recipes:navigation", [
  { ...meta("minimal-header", "navigation", "En-tête minimal", "Barre fine, dense faible.", ["minimal"]), Component: MinimalHeader },
  { ...meta("editorial-folio", "navigation", "Folio éditorial", "Liens numérotés, aéré.", ["editorial", "folio"]), Component: EditorialFolio },
  { ...meta("immersive-overlay", "navigation", "Overlay immersif", "Menu plein écran au clic.", ["immersive", "overlay"]), Component: ImmersiveOverlay },
  { ...meta("traditional-premium", "navigation", "Premium classique", "Liens centrés, soulignés.", ["classic"]), Component: TraditionalPremium },
]);

const projects = createRegistry<Recipe<ProjectsProps>>("recipes:projects", [
  { ...meta("visual-grid", "projects", "Grille visuelle", "Cartes média-en-tête.", ["grid", "media"]), Component: VisualGrid },
  { ...meta("editorial-index", "projects", "Index éditorial", "Sommaire en lignes.", ["index", "typographic"]), Component: EditorialIndex },
  { ...meta("horizontal-chapters", "projects", "Chapitres horizontaux", "Défilement horizontal.", ["horizontal", "immersive"]), Component: HorizontalChapters },
  { ...meta("case-study-sequence", "projects", "Séquence d'études de cas", "Sections alternées.", ["sequence", "editorial"]), Component: CaseStudySequence },
]);

const storytelling = createRegistry<Recipe<StorytellingProps>>("recipes:storytelling", [
  { ...meta("linear-sections", "storytelling", "Sections linéaires", "Une colonne, rythme régulier.", ["linear"]), Component: LinearSections },
  { ...meta("alternating-narrative", "storytelling", "Récit alterné", "Média/texte gauche-droite.", ["alternating"]), Component: AlternatingNarrative },
  { ...meta("chaptered-story", "storytelling", "Récit chapitré", "Grands numéros, typographique.", ["chaptered"]), Component: ChapteredStory },
  { ...meta("data-led-story", "storytelling", "Récit par les chiffres", "Ouvre sur un repère fort.", ["data"]), Component: DataLedStory },
  { ...meta("immersive-scroll", "storytelling", "Scroll immersif", "Sections plein écran.", ["immersive"]), Component: ImmersiveScroll },
]);

const conversion = createRegistry<Recipe<ConversionProps>>("recipes:conversion", [
  { ...meta("minimal-contact", "conversion", "Contact minimal", "Bloc centré sobre.", ["minimal"]), Component: MinimalContact },
  { ...meta("premium-inquiry", "conversion", "Demande premium", "Bande brand + carte d'action.", ["premium"]), Component: PremiumInquiry },
  { ...meta("qualification-form", "conversion", "Qualification", "Étapes + CTA.", ["qualification"]), Component: QualificationForm },
  { ...meta("appointment-ready", "conversion", "Prise de rendez-vous", "Carte RDV + appel.", ["appointment"]), Component: AppointmentReady },
]);

const layouts = createRegistry<Recipe<LayoutProps>>("recipes:layout", [
  { ...meta("editorial-layout", "layout", "Éditorial", "Colonne large, aéré.", ["editorial"]), Component: EditorialLayout },
  { ...meta("immersive-layout", "layout", "Immersif", "Pleine largeur.", ["immersive"]), Component: ImmersiveLayout },
  { ...meta("product-layout", "layout", "Produit", "Colonne resserrée.", ["product"]), Component: ProductLayout },
  { ...meta("institutional-layout", "layout", "Institutionnel", "Centré, formel.", ["institutional"]), Component: InstitutionalLayout },
]);
